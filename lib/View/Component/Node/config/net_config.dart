...
class NETConfiguration extends StatefulWidget {
  static String label = 'net';
  NETConfiguration({Key? key, required this.node}) : super(key: key);
  final Node node;

  @override
  NETConfigurationState createState() => NETConfigurationState();
}

class NETConfigurationState extends State<NETConfiguration> {
  bool loading = true;
  final  Map<String, dynamic> config = {};
  final GlobalKey<NodeConfigFormState> formStateKey = GlobalKey();
  NodeConfigForm form = NodeConfigForm(config:{},node: Node(),metricLabel: 'net',thresholdUnit: 'net',);
  ...
  _save() async {  
    ...
    print(payload) ; 
    FukuroResponse res = await NodeConfig.updateMetricConfigValues(widget.node.getNodeId(), NETConfiguration.label, payload);
    if(res.ok()){
      if(mounted){
        if( payload['threshold'] != 0){
          FukuroResponse res2 = await NodeConfig.enableNotification(widget.node.getNodeId(), NETConfiguration.label, payload['threshold']);
          if(!res2.ok()){
            if(mounted){
              String msg = res2.msg();
  ...
  _loadConfig() async {
    ...
    try {
      FukuroResponse res = await NodeConfig.loadMetricConfigurations(
          widget.node.getNodeId(), NETConfiguration.label);
      if (mounted) {
        if (res.ok()) {
          try{ 
        //    print(config);
