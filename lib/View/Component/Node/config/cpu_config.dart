...
class CPUConfiguration extends StatefulWidget {
  static String label = 'cpu';
  CPUConfiguration({Key? key, required this.node}) : super(key: key);
  final Node node;

  @override
  CPUConfigurationState createState() => CPUConfigurationState();
}

class CPUConfigurationState extends State<CPUConfiguration> {
  bool loading = true;
  final  Map<String, dynamic> config = {};
  final GlobalKey<NodeConfigFormState> formStateKey = GlobalKey();
  NodeConfigForm form = NodeConfigForm(config:{},node: Node(),metricLabel: 'cpu',thresholdUnit: 'MHz',);
  ...
  _initialize(){
    ...
    config['threshold'] = {};
    _loadConfig();
    form = NodeConfigForm(key: formStateKey, config:config,node: widget.node,metricLabel: 'cpu',thresholdUnit: '%(usage)',thresholdMax: 100,);
  }
  _save() async {  
    print('save');
    try{
    ...
    print(payload) ; 
    FukuroResponse res = await NodeConfig.updateMetricConfigValues(widget.node.getNodeId(), CPUConfiguration.label, payload);
    if(res.ok()){
      if(mounted){
        if( payload['threshold'] != 0){
          FukuroResponse res2 = await NodeConfig.enableNotification(widget.node.getNodeId(), CPUConfiguration.label, payload['threshold']);
          if(!res2.ok()){
            if(mounted){
              String msg = res2.msg();
  ...
  _loadConfig() async {
    ...
    try {
      FukuroResponse res = await NodeConfig.loadMetricConfigurations(
          widget.node.getNodeId(), CPUConfiguration.label);
      if (mounted) {
        if (res.ok()) {
          try{ 
        //    print(config);
