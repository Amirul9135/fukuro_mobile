...
class MEMConfiguration extends StatefulWidget {
  static String label = 'mem';
  MEMConfiguration({Key? key, required this.node}) : super(key: key);
  final Node node;

  @override
  MEMConfigurationState createState() => MEMConfigurationState();
}

class MEMConfigurationState extends State<MEMConfiguration> {
  bool loading = true;
  final  Map<String, dynamic> config = {};
  final GlobalKey<NodeConfigFormState> formStateKey = GlobalKey();
  NodeConfigForm form = NodeConfigForm(config:{},node: Node(),metricLabel: 'mem',thresholdUnit: ' ',);
  ...
  _initialize(){
    ...
    config['threshold'] = {};
    _loadConfig();
    form = NodeConfigForm(key: formStateKey, config:config,node: widget.node,metricLabel: 'mem',thresholdUnit: '%(used)',thresholdMax: 100,);
  }
  _save() async {  
    print('save');
    try{
    ...
    print(payload) ; 
    FukuroResponse res = await NodeConfig.updateMetricConfigValues(widget.node.getNodeId(), MEMConfiguration.label, payload);
    if(res.ok()){
      if(mounted){
        if( payload['threshold'] != 0){
          FukuroResponse res2 = await NodeConfig.enableNotification(widget.node.getNodeId(), MEMConfiguration.label, payload['threshold']);
          if(!res2.ok()){
            if(mounted){
              String msg = res2.msg();
  ...
  _loadConfig() async {
    ...
    try {
      FukuroResponse res = await NodeConfig.loadMetricConfigurations(
          widget.node.getNodeId(), MEMConfiguration.label);
      if (mounted) {
        if (res.ok()) {
          try{ 
        //    print(config);
